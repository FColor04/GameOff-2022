...
public class RaycastShoot : IAction
{
    ...
    public IEnumerator Execute(GunInstance gunInstance, Camera camera, GunAnimations animations)
    {
        ...
        for (int i = 0; i < burstCount; i++)
        {
            ...
            if (bulletTrail)
            {
                var startPosition = gunInstance.muzzlePositionTransform.position;
        ...
        //Spawn Muzzle Flash
        animations.PlayRecoil(recoilStrength);
        var MuzzleFlashVFXInstance = GameObject.Instantiate(MuzzleFlashVFX, gunInstance.muzzlePositionTransform);
